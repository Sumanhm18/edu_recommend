...
public class QuizController {
    ...
    public ResponseEntity<Map<String, Object>> submitQuizWithAI(
        ...
        try {
            ...
            // Get user's actual location information
            User user = userRepository.findById(userId).orElse(null);
            if (user != null && user.getDistrict() != null && !user.getDistrict().isEmpty()) {
                userProfile.put("district", user.getDistrict());
                userProfile.put("state", "Karnataka"); // Default state for now, can be enhanced later
            } else {
                userProfile.put("district", "Bangalore");
                userProfile.put("state", "Karnataka");
            }
    ...
    public ResponseEntity<?> getQuizStats(HttpServletRequest request) {
        try {
            ...
            if (!history.isEmpty()) {
                ...
                stats.put("improvement",
                        history.size() > 1
                                ? history.get(0).getPercentage() - history.get(history.size() - 1).getPercentage()
                                : 0);
            } else {
    ...
    public ResponseEntity<?> getQuizCategories() {
        ...
        streamInfo.put("Arts",
                "Languages, Social Sciences, History, Psychology - leads to Literature, Law, Social Work");
